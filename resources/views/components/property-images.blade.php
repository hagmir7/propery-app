...
fn($img) => $useMedia ? $img->getUrl() : Storage::url($img->path)
)->toArray();
$thumbUrls = $images->map(
fn($img) => $useMedia ? ($img->hasGeneratedConversion('thumb') ? $img->getUrl('thumb') : $img->getUrl()) :
...
<div class="max-w-7xl mx-auto" x-data="{
    ...
    switchImage(index) { this.currentIndex = index; },
    nextImage() { this.currentIndex = (this.currentIndex + 1) % this.images.length; },
    prevImage() { this.currentIndex = this.currentIndex === 0 ? this.images.length - 1 : this.currentIndex - 1; }
}">
    ...
    <div class="relative mb-6 rounded-xl overflow-hidden bg-gray-100 shadow-sm group">
        <div class="relative h-96 md:h-[500px]">
            ...
            <div x-show="currentIndex === {{ $index }}" x-transition:enter="transition ease-out duration-300"
                ...
                <img src="{{ $imageUrls[$index] }}" class="w-full h-full object-cover"
                    alt="Property image {{ $index + 1 }}">
            </div>
            @endforeach
            @else
            <img src="/api/placeholder/1200/600" class="w-full h-full object-cover" alt="No image">
            @endif
            ...
            <div
                ...
                <span x-text="currentIndex + 1"></span> / {{ $count }}
            </div>
            @endif
    ...
    {{-- No images placeholder grid --}}
    @if ($count === 0)
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 md:gap-4">
        @for ($i = 0; $i < 4; $i++) <div class="overflow-hidden rounded-lg border border-gray-200 shadow-sm">
            <img src="/api/placeholder/300/200" class="w-full h-24 md:h-28 object-cover opacity-50" alt="Placeholder">
    </div>
    @endfor
</div>
@endif
